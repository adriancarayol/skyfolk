...
def process_video_publication(file, publication_id, filename, user_id=None):
    ...
    try:
        publication = PublicationTheme.objects.select_related("board_theme").get(
            id=publication_id
        )
    ...
    try:
        ...
        try:
            notification = Notification.objects.create(
                ...
                description='<a href="{0}">Ver</a>'.format(
                    reverse_lazy("user_groups:group_theme", kwargs={"slug": theme.slug})
                ),
            )
        ...
        async_to_sync(channel_layer.group_send)(
            ...
            {"type": "new_publication", "message": {"content": content}},
        )

        async_to_sync(channel_layer.group_send)(
            theme.theme_channel, {"type": "new_publication", "message": data}
        )
...
def process_gif_publication(file, publication_id, filename, user_id=None):
    ...
    try:
        ...
        async_to_sync(channel_layer.group_send)(
            notification_channel(user.id), {"type": "new_publication", "message": data}
        )
    except Exception as e:
        pass
    finally:
