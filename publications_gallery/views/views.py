...
from publications_gallery.utils import (
...
class PublicationDetail(View):
    @staticmethod
    def _get_publication_and_descendants(request, photo, publications, publication_id, page):
    ...
    def get(self, request, publication_id):
        ...
        try:
            request_pub = PublicationPhoto.objects.select_related("board_photo").get(
                id=publication_id, deleted=False
            )
            if (
                    not request_pub.board_photo.is_public
                    and user.id != request_pub.board_photo.owner.id
            ):
                return redirect(
                    "photologue:photo-list", username=request_pub.board_photo.owner.username
                )
        ...
        publication = request_pub.get_descendants(include_self=True).annotate(
                ...
                "publication_photo_extra_content", "images", "videos", "tags"
            ).select_related("author", "board_photo", "parent").order_by("created")

        if not user.is_authenticated and author.privacity == Profile.ALL:
            return self._get_publication_and_descendants(request, request_pub.board_photo, publication, publication_id,
                                                         page)
...
def add_like(request):
    ...
    if request.POST:
        ...
        try:
            publication = PublicationPhoto.objects.get(
...
def load_more_descendants(request):
    ...
    """
    def get_descendants(publication_id, publication_list, board_photo):
    ...
    if request.is_ajax():
        ...
        pubs = publication.get_descendants().filter(deleted=False).order_by("created").annotate(
                likes=Count("user_give_me_like"),
                hates=Count("user_give_me_hate")).prefetch_related(
            "publication_photo_extra_content", "images", "videos", "parent__author"
        ).select_related("author", "board_photo", "parent")

        if not user.is_authenticated and board_owner.privacity == Profile.ALL:
            return get_descendants(pub_id, pubs, publication.board_photo)
