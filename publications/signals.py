    ...
    if action == "post_add":
        ...
        qs = Publication.objects.filter(author_id=instance.author_id).aggregate(
            likes=Count(
                Case(
                    When(
                        ~Q(user_give_me_like=instance.author)
                        & ~Q(user_give_me_like=None),
                        then=Value(1),
                    )
                )
            )
        ...
        if qs["likes"] >= 100:
            Award.objects.get_or_create(
                user=instance.author, badge=Badge.objects.get(slug="editor-recipe")
            )
        elif qs["likes"] >= 5000:
            Award.objects.get_or_create(
                user=instance.author, badge=Badge.objects.get(slug="pulitzer-recipe")
            )
...
def publication_handler(sender, instance, created, **kwargs):
    ...
    if total_pubs >= 10:
        Award.objects.get_or_create(
            user=instance.author, badge=Badge.objects.get(slug="10-pubs-reached")
        )
    elif total_pubs >= 1:
        Award.objects.get_or_create(
            user=instance.author, badge=Badge.objects.get(slug="first-publication")
        )
    ...
    if not instance.deleted:
        ...
        if instance.has_extra_content():  # Para publicaciones editadas
            ExtraContent.objects.filter(publication=instance).exclude(
...
def add_extra_content(instance):
    ...
    if (not link_url or len(link_url) <= 0) and instance.has_extra_content():
        ExtraContent.objects.filter(
            publication=instance
        ).delete()  # Borramos el extra content de esta
            ...
            notify.send(
                ...
                description='@{0} te ha mencionado en <a href="{1}">Ver</a>'.format(
                    ...
                    reverse_lazy(
                        ...
                        kwargs={"publication_id": instance.id},
                    ),
                ),
            )
