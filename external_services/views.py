...
class ListServiceView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['active_services'] = UserService.objects.filter(user=user).select_related('service')
        return context

    def get_queryset(self):
        user = self.request.user
        user_services = UserService.objects.filter(user=user).values_list('service_id', flat=True)
