...
from publications_gallery_groups.media_processor import (
...
class PublicationVideoView(AjaxableResponseMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        video = get_object_or_404(
            VideoGroup.objects.select_related("group"),
            id=request.POST.get("board_video", None),
        )
...
def video_publication_detail(request, publication_id):
    ...
    try:
        request_pub = PublicationGroupMediaVideo.objects.select_related(
            "board_video"
        ).get(id=publication_id, deleted=False)

        if (
            not request_pub.board_video.group.is_public
            and user.id != request_pub.board_video.group.owner_id
        ):
            if user.user_groups.filter(id=request_pub.board_video.group_id).exists():
                return redirect(
                    "photologue_groups:photo-list",
                    username=request_pub.board_video.owner.username,
                )
    ...
    try:
        publication = (
            ...
            .prefetch_related(
                "publication_group_multimedia_video_extra_content",
                "images",
                "videos",
                "tags",
            )
...
def add_video_like(request):
    ...
    if request.POST:
        ...
        try:
            publication = PublicationGroupMediaVideo.objects.get(
...
def load_more_video_descendants(request):
    ...
    if request.is_ajax():
        ...
        return render(
            request, "photologue_groups/videos/ajax_load_replies.html", context=context
        )
    return HttpResponseForbidden()
