...
from .models import PublicationGroupMediaPhoto, ExtraContentPubPhoto
...
class PublicationGalleryTestCase(TestCase):
    def setUp(self):
        ...
        p = Photo.objects.create(
            title="yee", owner=u, image="skyfolk/static/img/nuevo.png"
        )
        PublicationGroupMediaPhoto.objects.create(
            content="example_content", author=u, board_photo=p
        )

    def test_parse_content(self):
        pub = PublicationGroupMediaPhoto.objects.get(author__username="example")
        pub.parse_content()
        self.assertIsNotNone(pub, pub.content)

    def test_extra_content(self):
        pub = PublicationGroupMediaPhoto.objects.filter(
            author__username="example"
        ).first()
        e = ExtraContentPubPhoto.objects.create(
    ...
    def test_delete_extra_content(self):
        ...
        ExtraContentPubPhoto.objects.create(
            publication=pub, title="Extra content example"
        )
        pub.delete()
