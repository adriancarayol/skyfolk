...
class PublicationGalleryTestCase(TestCase):
    def setUp(self):
        ...
        PublicationPhoto.objects.create(content='example_content', p_author=u, board_photo=p)

    def test_parse_content(self):
        pub = PublicationPhoto.objects.get(p_author__username='example')
        pub.parse_content()
        self.assertIsNotNone(pub, pub.content)

    def test_extra_content(self):
        pub = PublicationPhoto.objects.filter(p_author__username="example").first()
