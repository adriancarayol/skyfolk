...
@receiver(
    post_save, sender=PublicationGroupMediaPhoto, dispatch_uid="photo_publication_save"
)
    ...
    if not instance.deleted:
        ...
        if instance.has_extra_content():  # Para publicaciones editadas
            ExtraContentPubPhoto.objects.filter(publication=instance.id).exclude(
                url=instance.publication_photo_extra_content.url
            ).delete()
        else:
...
def add_extra_content(instance):
    ...
    if (not link_url or len(link_url) <= 0) and instance.has_extra_content():
        ...
        if instance.has_extra_content():
            publication_photo_extra_content = (
                instance.publication_group_multimedia_photo_extra_content
            )
...
def decrease_affinity(instance):
    try:
        relation = RelationShipProfile.objects.get(
            ...
            to_profile=instance.board_photo.owner.profile,
            type=FOLLOWING,
        )
