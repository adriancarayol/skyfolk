...
urlpatterns = [
    ...
    url(
        r"^multimedia/submit_zip/$",
        require_POST(login_required(upload_zip_form)),
        name="upload-zip",
    ),
    url(
        r"^multimedia/submit_photo/$",
        require_POST(login_required(upload_photo)),
        name="upload-photo",
    ),
    url(
        r"^multimedia/submit_video/$",
        require_POST(login_required(upload_video)),
        name="upload-video",
    ),
    url(
        r"^multimedia/collection/by(?P<slug>[\w-]+)/(?P<tag_slug>[\w-]+)/$",
        collection_list,
        name="collection-list",
    ),
    url(r"^(?P<slug>[\w-]+)/multimedia/$", photo_list, name="photo-list"),
]
