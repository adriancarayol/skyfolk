...
def send_recommendation_via_email():
    ...
    for user in nodes:
        results, meta = db.cypher_query(
            "MATCH (u1:NodeProfile)-[:INTEREST]->(tag:TagProfile)<-[:INTEREST]-(u2:NodeProfile) WHERE NOT (u1:NodeProfile)-[:FOLLOW]->(u2:NodeProfile) AND NOT (u2:NodeProfile)-[:BLOCK]->(u1:NodeProfile) AND NOT (u1:NodeProfile)-[:BLOCK]->(u2:NodeProfile) AND u1.user_id=%d AND NOT u2.privacity='N' RETURN u2, COUNT(tag) AS score ORDER BY score DESC LIMIT %d" %
            (user.id, 25))
