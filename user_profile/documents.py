...
class ProfileDocument(DocType):
    ...
    tags = fields.NestedField(
        properties={"slug": fields.StringField(), "name": fields.StringField()}
    )
    ...
    def prepare_followers_count(self, instance):
        return RelationShipProfile.objects.filter(
            to_profile=instance, type=FOLLOWING
        ).count()
    ...
    def prepare_exp_count(self, instance):
        awards = (
            Award.objects.filter(user__profile__id=instance.id)
