...
class TemplateTagTest(TestCase):
    def setUp(self):
        ...
        self.guides = [
            Guide.objects.create(
                html='<div>Hello test {0}! <img src="{{static}}images/cool-image.png"></div>'.format(
                    i
                ),
    ...
    def test_user_guide_tags_no_filters(self):
        ...
        r = HttpRequest()
        r.user = self.users[0]
        c = Context({"request": r, "csrf_token": "1234"})
        ...
        self.assertTrue(
            "Hello test 4!" not in rendered
        )  # Should not have rendered 6 guides
        self.assertTrue(
            "Hello test 3!" not in rendered
        )  # Should not have rendered 7 guides
        self.assertTrue(
            "Hello test 2!" not in rendered
        )  # Should not have rendered 8 guides
        self.assertTrue(
            "Hello test 1!" not in rendered
        )  # Should not have rendered 9 guides
        self.assertTrue(
            "Hello test 0!" not in rendered
        )  # Should not have rendered 10 guides
        self.assertTrue(
            "django-user-guide.css" in rendered
        )  # Should have django-user-guide style sheet
        self.assertTrue(
            "django-user-guide.js" in rendered
        )  # Should have django-user-guide script
        self.assertTrue(
            "custom-style.css" in rendered
        )  # Should have custom style sheet
        self.assertTrue("custom-script.js" in rendered)  # Should have custom script
        self.assertTrue(
            '<img src="/static/images/cool-image.png">' in rendered
        )  # Should have expanded {static}

    def test_user_guide_tags_guide_name_filter(self):
        t = Template("{% load user_guide_tags %}{% user_guide guide_name=guide_name %}")
        r = HttpRequest()
        r.user = self.users[0]
        c = Context({"request": r, "guide_name": "Test Guide 1"})
    ...
    def test_user_guide_tags_guide_tag_filter(self):
        ...
        r.user = self.users[0]
        c = Context(
            {"request": r, "guide_tags": ["tag0", "tag1"], "csrf_token": "1234"}
        )
