...
class News(ListView):
    ...
    def get_affinity_users(self):
        ...
        """
        n = NodeProfile.nodes.get(title=self.request.user.username)
        pk_list = [u.title for u in n.get_favs_users()]
        return User.objects.filter(username__in=pk_list)

    def get_recommendation_users(self, offset=0):
    ...
    def get_queryset(self):
        ...
        try:
            ...
            pk_list = [x.title for x in n.get_favs_users(offset=0, limit=20)]
        except Exception as e:
            pass

        try:
            # ids de usuarios recomendados
            pk_list.extend([u.title for u in self.get_recommendation_users(offset=offset)])
        except Exception as e:
            pass
        ...
        try:
            publications = Publication.objects.filter(
                ((Q(board_owner__profile__in=following) |
                  Q(board_owner__username__in=pk_list)) & (
        ...
        try:
            photos = Photo.objects.filter(((Q(owner__profile__in=following)
                                            | Q(owner__username__in=pk_list)) & ~Q(
        ...
        try:
            entries_q = (Q(user__username__in=pk_list) | Q(user__profile__in=following)) \
        ...
        if len(videos) <= 0:
            videos = Video.objects.filter(owner__username__in=extended_list)[offset:limit]

        if len(publications) <= 0:
            publications = Publication.objects.filter(board_owner__username__in=extended_list)[offset:limit]

        if len(placeholders) <= 0:
            entries_q = Q(user__username__in=extended_list) & Q(layout_uid='profile', workspace=None)
            dashboard_entries = DashboardEntry._default_manager \
                                    .filter(entries_q) \
                                    .select_related('workspace', 'user') \
                                    .order_by('placeholder_uid', 'position')[offset:limit]
