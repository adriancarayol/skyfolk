...
class NewTestClass(TestCase):
    ...
    def test_get_following_publications(self):
        ...
        following = RelationShipProfile.objects.filter(
            Q(from_profile=u.profile) & ~Q(type=BLOCK)
        ).values("to_profile_id")
        self.assertTrue(
            Publication.objects.filter(author__profile__in=following).exists()
        )
    ...
    def test_get_not_blocked_publications(self):
        ...
        users_not_blocked_me = RelationShipProfile.objects.filter(
            to_profile=u.profile, type=BLOCK
    ...
    def test_get_following_board_publications(self):
        ...
        self.assertTrue(
            Publication.objects.filter(board_owner__profile__in=following).exists()
        )
    ...
    def test_get_not_blocked_board_publications(self):
        ...
        self.assertFalse(
            Publication.objects.exclude(
                board_owner__profile__in=users_not_blocked_me
            ).exists()
        )
