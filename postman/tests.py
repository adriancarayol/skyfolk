...
if VERSION >= (1, 10):
    TransactionViewTest = override_settings(ROOT_URLCONF="postman.urls_for_tests")(
        TransactionViewTest
    )
...
class ViewTest(BaseTest):
    ...
    def check_read(self, view_name, success_msg, pk, field_value=True):
        ...
        self.check_contrib_messages(response, success_msg)
        self.check_status(
            Message.objects.get(pk=pk), status=STATUS_ACCEPTED, is_new=not field_value
        )
        self.check_status(Message.objects.get(pk=pk + 1), status=STATUS_ACCEPTED)
        self.check_status(
            Message.objects.get(pk=pk + 2),
            status=STATUS_ACCEPTED,
            is_new=not field_value,
        )
        self.check_status(Message.objects.get(pk=pk + 3), status=STATUS_ACCEPTED)
        ...
        self.check_contrib_messages(response, success_msg)
        self.check_status(
            Message.objects.get(pk=pk), status=STATUS_ACCEPTED, is_new=field_value
        )
        self.check_status(
            Message.objects.get(pk=pk + 1), status=STATUS_ACCEPTED
        )  # unchanged
        self.check_status(
            Message.objects.get(pk=pk + 2), status=STATUS_ACCEPTED, is_new=field_value
        )
        self.check_status(
            Message.objects.get(pk=pk + 3), status=STATUS_ACCEPTED
        )  # unchanged
        # missing payload
        response = self.client.post(url, follow=True)
        self.assertRedirects(response, reverse("postman:inbox"))
    ...
    def check_read_conversation(self, view_name, pk, field_value=True):
        ...
        self.check_status(
            m2, status=STATUS_ACCEPTED, parent=m1, is_replied=True, thread=m1
        )
        self.check_status(
            Message.objects.get(pk=pk + 2),
            status=STATUS_ACCEPTED,
        ...
        m1 = Message.objects.get(pk=pk)
        self.check_status(
            m1, status=STATUS_ACCEPTED, is_new=field_value, is_replied=True, thread=m1
        )
        m2 = Message.objects.get(pk=pk + 1)
        self.check_status(
            m2, status=STATUS_ACCEPTED, parent=m1, is_replied=True, thread=m1
...
class UtilsTest(BaseTest):
    ...
    def check_email(
        self, message_template_name, recipient_list, object, action, site, parts=None
    ):
        mail.outbox = []
    ...
    def test_email(self):
        ...
        self.check_email(
            "email_txt_only", recipient_list, m, action, site, "Text only\n"
        )
        self.check_email(
            "email_html_only",
            recipient_list,
