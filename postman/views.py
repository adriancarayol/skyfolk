...
class UpdateDualMixin(UpdateMessageMixin):
    def _action(self, user, filter):
        ...
        recipient_rows = (
            ...
            .filter(
                **{
                    "recipient_{0}{1}".format(
                        self.field_bit, criteria_key
                    ): criteria_val
                }
            )
        ...
        sender_rows = (
            Message.objects.as_sender(user, filter)
            .filter(
                **{"sender_{0}{1}".format(self.field_bit, criteria_key): criteria_val}
            )
