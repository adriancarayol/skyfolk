...
class EmojiTest(TestCase):
    ...
    def test_if_no_valid_emoji_return_unchanged(self):
        self.assertEqual(
            Emoji.replace(":ducklingdrinkingcoffee:"), ":ducklingdrinkingcoffee:"
        )
...
class EmojiTemplateTagTest(TestCase):
    def test_emoji_replace_tag(self):
        ...
        self.assertEqual(res.status_code, 200)
        self.assertIn(
            '<img src="/static/emoji/img/%2B1.png" '
            'alt="+1" title="+1" class="emoji">',
            str(res.content),
        )
    ...
    def test_emoji_replace_unicode_tag(self):
        ...
        self.assertIn(
            (
                '<img src="/static/emoji/img/kiss.png" '
                + 'alt="💋" title="kiss" '
                + 'class="emoji">'
            ),
    ...
    def test_emoji_load_tag(self):
        ...
        self.assertEqual(res.status_code, 200)
        self.assertTrue("Emoji.setDataUrl('/all.json').load();" in str(res.content))
    ...
    def test_xss_replace_unicode(self):
        ...
        self.assertEqual(res.status_code, 200)
        correct_string = (
            '&lt;em&gt;<img src="/static/emoji/img/shit.png" '
    ...
    def test_xss_replace_emoji(self):
        ...
        self.assertEqual(res.status_code, 200)
        correct_string = (
            '&lt;em&gt;<img src="/static/emoji/img/dog.png" '
...
class EmojiUnicodeTest(UnicodeTestBase):
    ...
    def test_replace_unicode_in_string_with_images(self):
        self.assertIn(
            "kiss.png", Emoji.replace_unicode("I send a {0}!".format(self.UNICODE_KISS))
        )
    ...
    def test_remove_unicode_control_from_replace_unicode(self):
        ...
        self.assertEqual(
            ...
            '<img src="/static/emoji/img/v.png" alt="✌" ' 'title="v" class="emoji">',
        )
...
class EmojiHtmlEntitiesTest(UnicodeTestBase):
    ...
    def test_should_replace_integer_encoded_unicode(self):
        self.assertEqual(
            Emoji.replace_html_entities(self.INTEGER_KISS), self.UNICODE_KISS
        )

    def test_should_replace_hex_encoded_unicode(self):
        self.assertEqual(Emoji.replace_html_entities(self.HEX_KISS), self.UNICODE_KISS)

    def test_should_replace_short_hex_encoded_unicode(self):
        self.assertEqual(
            Emoji.replace_html_entities(self.HEX_KISS_SHORT), self.UNICODE_KISS
        )
