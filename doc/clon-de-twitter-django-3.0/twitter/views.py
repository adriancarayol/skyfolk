...
def buscar(request, page=1):
    ...
    if request.GET.has_key('busqueda'):
        ...
        t = Tweet.objects.filter(activo=True, contenido__icontains=request.GET['busqueda']).order_by('-fecha')[
            n:n + TWEETS_EN_PAGE]
...
def randuser(user, n=1):
    ...
    final = []
    for x in range(0, n):
        """ Se vuelve a ejectutar salvo que se llegue al límite o que se haya
