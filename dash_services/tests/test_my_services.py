...
from dash_services.my_services import MyService


class MyServiceTest(TestCase):

    def setUp(self):
        self.package = "th_services.th_rss"
        self.module_name = 'my_rss'
        self.service_name = 'ServiceRss'
        self.full_name = "th_services.th_rss.my_rss.ServiceRss"

    def test_full_name(self):
        service_long = MyService.full_name('th_services.th_rss')
        self.assertEqual(self.full_name, service_long)
        self.assertTrue(service_long in settings.TH_SERVICES)

    def test_module_name(self):
        module_name = MyService.module_name('th_services.th_rss')
        self.assertEqual(self.module_name, module_name)

    def test_service_name(self):
        service_name = MyService.service_name('th_services.th_rss')
